...
hip_configs = [
    ...
    triton.Config({'BLOCK_SIZE_M': 256, 'BLOCK_SIZE_N': 256, 'BLOCK_SIZE_K': 64, 'GROUP_SIZE_M': 6, 'NUM_STAGES': 2}
                  | {'matrix_instr_nonkdim': 16, 'waves_per_eu': 0}, num_warps=4, num_stages=2)
]
...
def matmul_kernel_pipelined_mi300(a_ptr, b_ptr, c_ptr, M, N, K, stride_am, stride_ak,  #
    ...
    i = 0
    a_smem_view_left = tlx.local_view(buffers_A, i)
    b_smem_view_left = tlx.local_view(buffers_B, i)
    token_a = tlx.async_load(a_ptrs, a_smem_view_left, mask=offs_k[None, :] < K - i * BLOCK_SIZE_K)
    token_b = tlx.async_load(b_ptrs, b_smem_view_left, mask=offs_k[:, None] < K - i * BLOCK_SIZE_K)
    a_ptrs += BLOCK_SIZE_K * stride_ak
    b_ptrs += BLOCK_SIZE_K * stride_bk
    tlx.async_load_commit_group([token_a, token_b])

    i = 1
    a_smem_view_right = tlx.local_view(buffers_A, i)
    b_smem_view_right = tlx.local_view(buffers_B, i)
    token_a = tlx.async_load(a_ptrs, a_smem_view_right, mask=offs_k[None, :] < K - i * BLOCK_SIZE_K)
    token_b = tlx.async_load(b_ptrs, b_smem_view_right, mask=offs_k[:, None] < K - i * BLOCK_SIZE_K)
    a_ptrs += BLOCK_SIZE_K * stride_ak
    b_ptrs += BLOCK_SIZE_K * stride_bk
    tlx.async_load_commit_group([token_a, token_b])

    tlx.async_load_wait_group(1)
    a_left_reg = tlx.local_load(a_smem_view_left)
    b_left_reg = tlx.local_load(b_smem_view_left)
    ...
    for k in tl.range(NUM_STAGES - 1, K_ITERS, num_stages=0):
        ...
        token_a = tlx.async_load(a_ptrs, a_k_smem_view, mask=offs_k[None, :] < K - k * BLOCK_SIZE_K)
        token_b = tlx.async_load(b_ptrs, b_k_smem_view, mask=offs_k[:, None] < K - k * BLOCK_SIZE_K)
        tlx.async_load_commit_group([token_a, token_b])
