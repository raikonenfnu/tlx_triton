...
def matmul_kernel_pipelined_mi300(a_ptr, b_ptr, c_ptr, M, N, K, stride_am, stride_ak,  #
    ...
    for i in tl.range(0, NUM_STAGES - 1, loop_unroll_factor=NUM_STAGES - 1):
        ...
        token_a = tlx.async_load(a_ptrs, a_smem_view, mask=offs_k[None, :] < K - i * BLOCK_SIZE_K)
        token_b = tlx.async_load(b_ptrs, b_smem_view, mask=offs_k[:, None] < K - i * BLOCK_SIZE_K)
