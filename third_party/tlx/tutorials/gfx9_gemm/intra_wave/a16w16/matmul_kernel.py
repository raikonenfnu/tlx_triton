...
STATUS / PERF NOTE
------------------
Correctness matches torch on all shapes. Performance does NOT (yet) reproduce
the Gluon reference. On gfx950 (MI350), fp16, do_bench:

    M     N     K       rocBLAS   this kernel   (TLX a16w16/v9 ref)
    2048  8192  18432   ~1169T    ~467T         ~1020T
    2048  8192  8192    ~1168T    ~449T         ~975T
    4096  8192  8192    ~1202T    ~494T         ~1020T

Root cause (investigated): compute work is identical to v9 (256 v_mfma/warp) and
AGPRs are correctly forbidden (240 VGPR / 0 spill via llvm_fn_attrs), but this
repo's TLX `warp_pipeline_stage` lowering emits full `s_barrier`s at every stage
boundary (22 vs v9's 15). With 8 regions the two wave groups end up running
serially instead of a stage apart, so the inter-wave overlap the design relies on
never materializes (dropping the ping-pong wrappers actually *raises* this to
~630T). Reproducing the reference will require the warp-pipeline pass to emit the
conditional one-stage-ahead barriers rather than full barriers.
"""
