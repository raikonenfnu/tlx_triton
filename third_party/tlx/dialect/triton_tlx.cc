  ...
  builder_cls
      ...
      .def(
          ...
          [](TritonOpBuilder &self, Value &v, Attribute &encoding,
            ...
            if (auto type = dyn_cast<ttg::MemDescType>(v.getType())) {
              ...
              // A pinned require wraps the encoding as #tlx.user_layout so it
              // anchors through the layout passes (won't be rewritten to a
              // "preferred" layout); the default wraps as
              // #tlx.no_verify_layout.
      ...
      .def("make_padded_shared_encoding_attr_with_bases",
           [](TritonOpBuilder &self, std::vector<unsigned> intervals,
             ...
             // Explicit-linear-component form of #ttg.padded_shared, mirroring
             // the {offset = [...], block = [...]} textual variant. Lets a TLX
             // author pin a swizzled shared layout (row/col permutation) rather
             // than the identity {order, shape} form.
             assert(intervals.size() == paddings.size() &&
                    "intervals/paddings size mismatch");
             auto context = self.getBuilder().getContext();
             llvm::SmallVector<std::pair<unsigned, unsigned>> intervalPads;
