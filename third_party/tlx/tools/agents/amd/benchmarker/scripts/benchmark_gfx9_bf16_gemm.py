...
def _streamk_support(shape: Shape) -> tuple[bool, str]:
    valid = (shape.op == "mm" and shape.layout_a == "row" and shape.layout_b == "column" and shape.m % 256 == 0
             and shape.n % 256 == 0 and shape.k >= 128 and shape.k % 128 == 0)
    return valid, "requires mm, row/column operands, and M/N/K divisible by 256/256/128"


def _interwave_support(shape: Shape) -> tuple[bool, str]:
    valid = shape.op == "mm" and shape.layout_a == "row" and shape.layout_b == "column"
    return valid, "direct-to-LDS path requires row-major A and column-major B"
...
def _candidate_config(name: str, shape: Shape) -> dict | None:
    ...
    if name in {"interwave", "addmm_interwave"}:
        from triton.language.extra.tlx.tutorials.gfx9_gemm.inter_wave.a16w16.matmul_kernel import choose_tile
...
def _markdown(report: dict, command: str) -> str:
    ...
    lines = [
        ...
        "## Recommendation",
        "",
        f"Use the PyTorch operation for this suite. No TLX winner cleared a 1% decision threshold "
        f"({decisive_tlx_wins} decisive TLX wins); the supplied-weight aggregate is "
        f"`{weighted_tlx / weighted_torch:.3f}x` faster for PyTorch versus the best correct TLX path per shape.",
        "",
