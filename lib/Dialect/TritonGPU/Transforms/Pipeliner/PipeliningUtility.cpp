...
void mlir::triton::combineRedundantWaitOps(
  ...
  for (auto waitOp : waitOps) {
    ...
    // branching op (forOp, ifOp, whileOp), or local-memory read in between the
    // waits. A local_load after an async_wait is the consumption point for data
    // made visible by that wait; merging a later, stricter wait into the earlier
    // one can turn a staggered software pipeline tail into a full drain.
    while (next &&
           !isa<ttg::AsyncCommitGroupOp, ttg::LocalLoadOp,
